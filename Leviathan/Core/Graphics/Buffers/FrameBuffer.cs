...
namespace Leviathan.Core.Graphics.Buffers
{
    public class FrameBuffer : IDisposable
    {
        ...
        private FrameBuffer()
        {
            Handle = Context.GLContext.GenFramebuffer();
        }

        public void Bind()
        {
            Context.GLContext.BindFramebuffer(Silk.NET.OpenGL.FramebufferTarget.Framebuffer, this.Handle);
            bound = this;
        }

        public static void UnbindAll()
        {
            Context.GLContext.BindFramebuffer(Silk.NET.OpenGL.FramebufferTarget.Framebuffer, 0);
            bound = null;
        }

        public void CopyDepthBufferToMainBuffer()
        {
            Context.GLContext.BlitFramebuffer(0, 0, BufferSize.X, BufferSize.Y, 0, 0, BufferSize.X, BufferSize.Y, Silk.NET.OpenGL.ClearBufferMask.DepthBufferBit, Silk.NET.OpenGL.BlitFramebufferFilter.Nearest);
        }
        ...
        public void Destroy()
        {
            ...
            {
                rbuffer.Destroy();
                Context.GLContext.DeleteFramebuffer(this.Handle);
                this.Handle = 0;
            }
        }
        ...
        public String GetFrameBufferStatus()
        {
            ...
            Silk.NET.OpenGL.GLEnum status = Context.GLContext.CheckFramebufferStatus(Silk.NET.OpenGL.FramebufferTarget.Framebuffer);
        ...
        private static FrameBuffer CreateDeferredBuffer(Vector2i size)
        {
            ...
            //Albedo buffer
            Context.GLContext.FramebufferTexture3D(Silk.NET.OpenGL.FramebufferTarget.Framebuffer,
                Silk.NET.OpenGL.FramebufferAttachment.ColorAttachment0,
        ...
        private static FrameBuffer CreateForwardBuffer(Vector2i size)
        {
            ...
            albedoBuffer.SetFilterMode(MinMagSetting.NEAREST);
            albedoBuffer.SetWrapMode(TextureWrapSetting.BORDER_CLAMP);
            Context.GLContext.FramebufferTexture2D(Silk.NET.OpenGL.FramebufferTarget.Framebuffer,
                Silk.NET.OpenGL.FramebufferAttachment.ColorAttachment0,
                Silk.NET.OpenGL.GLEnum.Texture2D,
                albedoBuffer.Handle,
                0);
    ...
    public class RenderBuffer : IDisposable
    {
        ...
        {
            this.Handle = Context.GLContext.GenRenderbuffer();
            Context.GLContext.RenderbufferStorage(Silk.NET.OpenGL.RenderbufferTarget.Renderbuffer, 
                iFormat, 
                (uint)buff_size.X, 
                (uint)buff_size.Y);
            
            Context.GLContext.FramebufferRenderbuffer(Silk.NET.OpenGL.FramebufferTarget.Framebuffer,
                fAttachment,
        ...
        public void Bind()
        {
            if(Handle != 0)
            {
                Context.GLContext.BindRenderbuffer(Silk.NET.OpenGL.GLEnum.Renderbuffer, Handle);
                Bound = this;
            } else
            {
        ...
        public static void UnbindAll()
        {
            Context.GLContext.BindRenderbuffer(Silk.NET.OpenGL.GLEnum.Renderbuffer, 0);
            Bound = null;
        }
        ...
        public void Destroy()
        {
            if(Handle != 0)
            {
                ...
                Context.GLContext.DeleteRenderbuffer(Handle);
                Handle = 0;
            }
        }
