...
namespace Leviathan.Math
{
    ...
    public struct Vector3d
    {
        ...
        public static Vector3d Parse(string[] tokens, int begin, NumberFormatInfo format = null)
        {
            if (tokens.Length < 3)
            {
                throw new ArgumentException($"Too few arguments given to parse Vector3d: Expected >= 3 and got {tokens.Length}");
            }

            if (tokens.Length - begin < 0)
            {
                throw new ArgumentException($"Too few arguments given to parse Vector3d: Expected >= 3 parseable tokens and got {tokens.Length - begin} accounting for start index");
            }
